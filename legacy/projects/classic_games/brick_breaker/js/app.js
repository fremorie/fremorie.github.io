...
function drawRect1(leftX, topY, width, height, drawColor) {
  ctx1.fillStyle = drawColor;
  ctx1.fillRect(leftX, topY, width, height);
}
...
function drawCircle1(centerX, centerY, radius, drawColor) {
  ctx1.fillStyle = drawColor;
  ctx1.beginPath();
  ctx1.arc(centerX, centerY, radius, 0, Math.PI*2, true);
  ctx1.fill()
}

function renderText() {
  // draw text header
  ctx1.font = '60px "Bungee Inline"';
  ctx1.textAlign = "center";
  ctx1.fillStyle = colors.deepDeepBlue;
  ctx1.fillText('BRICK BREAKER', canvas1.width/2, (borderWidth + upperPanelHeight)/1.5);
